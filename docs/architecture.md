...
- `PresenceIngester` is the heartbeat handler: it stamps `devices.last_seen`, marks the device online, and records the heartbeat in the in-memory presence model. It also persists the per-heartbeat extras the agent rides along (network fields, reported/rolled-back version) and — from a rolled agent (#157) — the device's system `boot_time` + previous-shutdown cause: a changed `boot_time` versus the device's stored value records a row in `device_reboots` and advances the device's last boot state, so CP can later tell a **reboot** (boot_time changed across an offline window) from a **network/MQTT blip** (unchanged). See `docs/CONTEXT.md` → *Offline reason*.
...
| `configupdate` | Wire types + validation for the cp → agent `config.update` cmd (Phase 2 slice 2) — `Request`, `Parse`, `ParseAllowList`, `ParseInterval`. Strict two-field whitelist per ADR-028. Both the agent handler and the CP API handler import this so validation rules can't drift. | Built (Phase 2 allow-list overrides) |
| `logtail` | Wire types + validation for the cp → agent `log.tail` cmd (Phase 2 slice 3) — `Request{log_name, lines}`, `Response{content, truncated, truncated_from}`, stable error codes (`log_tail.unknown_log`, `log_tail.binary_file`, etc.). Same shared-validation pattern as `configupdate`. | Built (Phase 2 log-tail) |
...
Phase 2's second slice — per-device allow-list overrides — landed and **establishes the first downward CP→agent message pattern**. An operator's `PUT /devices/{id}/service-config` persists the override on `devices.service_allow_list_override` + `devices.service_status_interval_override` (migration 012), then publishes a `config.update` envelope on `devices/{id}/cmd` via AWS IoT data-plane (`internal/cp/iotpublisher`). The agent's dispatcher routes the cmd to a new `configupdate.Handler` (validation rules shared with the API via `internal/protocol/configupdate`); the handler delegates to a `ConfigUpdateApplier` that atomically rewrites `agent-config.json` (read-modify-write through `map[string]any` so unrelated fields are preserved) and hot-reloads the running `ServiceStatusCollector` + `ServiceStatusPublisher`. The agent ACKs on `devices/{id}/cmd-result`; a new IoT Rule → SQS feeds `cp-ingest`'s `CmdResultIngester`, which routes by `envelope.Result.Type` and stamps `service_config_last_applied_*` on the device row for `config.update` ACKs. The per-device GET returns a `service_config` block; the dashboard's Services Card shows an `(overridden)` / `(default)` badge and opens an `EditServicesModal` that polls until the agent's correlation_id round-trips. **The cmd channel is unsigned** for slice 2 — ADR-028 records the narrow Phase 2 carve-out from ADR-013's Phase 3 signed-command pipeline; only two whitelisted fields are accepted by the handler. Phase 3 wraps the same `config.update` envelope in the signed shape without rewriting the handler logic.
