...
| **Allow-list (service)** | The per-device list of launchd/systemd unit names the agent reports status for. Configured in the agent's JSON config (`service_allow_list`). Empty list disables service-status reporting entirely — the publisher isn't even constructed. |
