...
**Amendment (2026-06-27, [#172](https://github.com/emilejacobs/control-plane/issues/172)).** §2's "the root agent manages only the workload" and the Verification's "Colima auto-starts at reboot via the LaunchAgent" are **refined**: the `com.uknomi.colima` *user* LaunchAgent loads **non-deterministically at GUI login** — on some boots it never runs (no `/tmp/colima.log`), leaving Colima and the ALPR container down (observed on courtney, stmarys, speedway-57, bell-76 after reboot). There is no static config difference between devices that auto-start and those that don't (identical plist/perms/disable-state), so it can't be predicted or fixed in the plist. The agent — a root LaunchDaemon that loads **reliably** at boot and already reaches Colima via `launchctl asuser` — therefore also owns Colima **liveness**: on startup and every ~5 min it runs `colima status` and, if stopped, `colima start` (no args — resumes the saved profile incl. the §5 network flags) via the same asuser path. The user LaunchAgent remains the primary starter; the agent is the dependable backstop. So the agent now manages workload **and** VM liveness, not workload alone. `host_net_pressure` (#167) is unrelated but surfaced the broad reboot exposure that motivated this.
