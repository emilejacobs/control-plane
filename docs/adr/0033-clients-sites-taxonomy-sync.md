...
3. **Credential storage**: AWS Secrets Manager (`uknomi/cp/taxonomy-api-creds`, JSON `{"username","password"}` — a Cognito user provisioned by the API team in pool `us-east-1_c8l8JVBwU`). The `cp-taxonomy-sync` task role gets `secretsmanager:GetSecretValue` on the specific ARN. The secret was created out-of-band via AWS CLI 2026-05-26 (bootstrap); the implementation slice ([#18](https://github.com/emilejacobs/control-plane/issues/18)) imports it into [`infra/terraform-deploy/secrets.tf`](../../infra/terraform-deploy/secrets.tf) following the established pattern (`aws_secretsmanager_secret` + `aws_secretsmanager_secret_version` with `ignore_changes = [secret_string]`).
...
- API team coordination: Cognito user provisioned in pool `us-east-1_c8l8JVBwU`; credentials populated into `uknomi/cp/taxonomy-api-creds` (created 2026-05-26 via CLI; Terraform-imported during #18); sample `/brand` + `/brand/{id}/store` responses captured for fixture-based testing.
