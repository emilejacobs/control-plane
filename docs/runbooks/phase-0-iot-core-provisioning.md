...
>
> **2026-05-21: Phase 0 device decommissioned.** The Phase 0 `dev-mac-mini-emile` thing + cert that this runbook produced have been decommissioned (revoke cert, delete thing, uninstall agent). The same Mac is now the Wave 0 (Bench) device for Phase 1 and gets re-provisioned through the new install module against fresh Terraform-managed IoT Core resources. The device's role is bench-only — it enrolls/decommissions as needed for testing and is **not** a permanent fleet member. Do not re-run this manual runbook for any fleet device.
